...
void fitZNA(const char* AnalysisName){
  ...
  RooDataHist* fZNAEnergyAgainstEntriesRoofitH = new RooDataHist( "fZNAEnergyAgainstEntriesH",
                                                                  "",
                                                                  x,
                                                                  fZNAEnergyAgainstEntriesH
                                                                  );
  RooFitResult* fFitResult = fit->fitTo( *fZNAEnergyAgainstEntriesRoofitH,
                                          RooFit::Save(),
                                          Range(-190, 700)
                                         );
  new TCanvas( "ZNAEnergy", "ZNAEnergy", 900, 800 );
  ...
  frame->SetTitle(  Form(  ";ZNA Energy [a.u.];Counts / (%.0f a.u.)",
                           fZNAEnergyAgainstEntriesH->GetXaxis()->GetBinWidth(1)  )  );
  fZNAEnergyAgainstEntriesRoofitH->plotOn( frame, MarkerStyle(kFullCircle), MarkerSize(1)     );
  ...
  /* - Chi square computation.
     -
   */
  RooChi2Var chi2_lowstat("chi2_lowstat","chi2",*fit,*fZNAEnergyAgainstEntriesRoofitH) ;
  latex->DrawLatex(0.55,0.18,Form("      #tilde{#chi}^{2} = %.2f / %.2d = %.2f  ",
                                  chi2_lowstat.getVal(),
                                  fZNAEnergyAgainstEntriesRoofitH->numEntries() - fit->getParameters(*fZNAEnergyAgainstEntriesRoofitH)->selectByAttrib("Constant",kFALSE)->getSize(),
                                  chi2_lowstat.getVal()/( fZNAEnergyAgainstEntriesRoofitH->numEntries() - fit->getParameters(*fZNAEnergyAgainstEntriesRoofitH)->selectByAttrib("Constant",kFALSE)->getSize() )
                                   )
                                 );
  gPad->SaveAs("pngResults/fZNAEnergyAgainstEntries.png", "RECREATE");
}
